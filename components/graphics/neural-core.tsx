...
const chips = [
  { kicker: "agent.01", title: "Sales brain", meta: "intent · score", cls: "chip1" },
  { kicker: "agent.02", title: "Ops memory", meta: "context · sync", cls: "chip2" },
  { kicker: "agent.03", title: "CRM neural", meta: "graph · 24/7", cls: "chip3" },
  { kicker: "agent.04", title: "Portal auth", meta: "roles · 2FA", cls: "chip4" },
] as const;
...
export function NeuralCore() {
  ...
  useEffect(() => {
    if (phase === "type") {
      ...
      const id = window.setTimeout(() => setTyped(current.slice(0, typed.length + 1)), TYPE_SPEED);
      return () => window.clearTimeout(id);
    }
    if (phase === "hold") {
    ...
    if (phase === "erase") {
      ...
      const id = window.setTimeout(() => setTyped(current.slice(0, typed.length - 1)), ERASE);
      return () => window.clearTimeout(id);
    }
    if (phase === "next") {
  ...
  return (
    <div className={styles.neural} aria-hidden="true">
      ...
      {chips.map((chip) => (
        <article key={chip.kicker} className={`${styles.chip} ${chipClassMap[chip.cls]}`}>
